 ...
 * Standalone (kein ES-Module) — supabase.js ist hier inline enthalten.
 */
...
async function sbRefreshToken() {
  ...
  var data = await res.json();
  if (!res.ok) { _sbToken = ''; _sbRefresh = ''; throw new Error('Session abgelaufen — bitte neu einloggen.'); }
  _sbToken   = data.access_token;
  _sbRefresh = data.refresh_token;
...
Office.onReady(function() {
  ...
  if (cfg.email && cfg.password) {
    sbLogin(cfg.email, cfg.password).then(function() {
      show('readyContent');
